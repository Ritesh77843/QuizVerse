...
const activeConnections = new Map<string, Map<string, string>>();

// Track question timers: pin -> { interval, remaining, timeout? }
const questionTimers = new Map<string, { interval: NodeJS.Timeout; remaining: number; timeout?: NodeJS.Timeout }>();

// Track per-player answer streaks: pin -> nickname -> consecutiveCorrect
const playerStreaks = new Map<string, Map<string, number>>();

function clearQuestionTimer(pin: string) {
...
export const setupSocketIO = (io: Server) => {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join-game', async (data: { pin: string, nickname?: string, isHost?: boolean }) => {
    ...
    // ── START GAME ─────────────────────────────────────────────────
    socket.on('start-game', async (data: { pin: string }) => {
      const { pin } = data;
    ...
    // ── NEXT QUESTION ──────────────────────────────────────────────
    socket.on('next-question', async (data: { pin: string }) => {
      const { pin } = data;
      const game = await prisma.gameSession.findUnique({
        where: { pin },
        include: { quiz: { include: { questions: { include: { options: true }, orderBy: { position: 'asc' } } } }, players: true }
      ...
      if (nextIdx >= game.quiz.questions.length) {
        ...
        await prisma.gameSession.update({ where: { pin }, data: { status: 'ENDED', endedAt: new Date() } });
        const players = await prisma.player.findMany({ where: { sessionPin: pin }, orderBy: { score: 'desc' } });
      ...
      await prisma.gameSession.update({ where: { pin }, data: { currentQuestionIndex: nextIdx } });
      io.to(pin).emit('game-state-changed', {
    ...
    socket.on('end-question-timer', (data: { pin: string }) => {
      const { pin } = data;
    ...
    // ── PAUSE / RESUME ─────────────────────────────────────────────
    socket.on('pause-game', async (data: { pin: string }) => {
      const { pin } = data;
    ...
    socket.on('resume-game', async (data: { pin: string }) => {
      const { pin } = data;
      const game = await prisma.gameSession.update({
    ...
    // ── LOCK / UNLOCK LOBBY ────────────────────────────────────────
    socket.on('lock-game', async (data: { pin: string }) => {
      const { pin } = data;
      const game = await prisma.gameSession.update({
        where: { pin }, data: { isLocked: true } as any, include: { players: true }
      });
      io.to(pin).emit('game-state-changed', { pin, status: (game as any).status, isLocked: true, playerCount: game.players.length });
      console.log(`Game ${pin} locked by host`);
    });

    socket.on('unlock-game', async (data: { pin: string }) => {
      const { pin } = data;
      const game = await prisma.gameSession.update({
        where: { pin }, data: { isLocked: false } as any, include: { players: true }
      });
      io.to(pin).emit('game-state-changed', { pin, status: (game as any).status, isLocked: false, playerCount: game.players.length });
      console.log(`Game ${pin} unlocked by host`);
    });

    // ── END GAME ───────────────────────────────────────────────────
    socket.on('end-game', async (data: { pin: string }) => {
      const { pin } = data;
    ...
    socket.on('submit-answer', async (data: {
      ...
      try {
        ...
        const player = playerId
          ...
          : await prisma.player.findFirst({ where: { sessionPin: pin, nickname } });

        // ── STREAK TRACKING ─────────────────────────────────────────
        const playerKey = player?.nickname ?? nickname ?? '';
        if (!playerStreaks.has(pin)) playerStreaks.set(pin, new Map());
        ...
        if (isCorrect) {
          ...
          if (newStreak === 3) streakBonus = 200;
          else if (newStreak === 5) streakBonus = 500;
          else if (newStreak > 5 && newStreak % 5 === 0) streakBonus = 500;
        }
...
function cleanupGameData(pin: string) {
  playerStreaks.delete(pin);
  activeConnections.delete(pin);
}

// Helper to emit a question to the room
function emitQuestion(io: Server, pin: string, questions: any[], idx: number) {
